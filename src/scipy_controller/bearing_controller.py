...
d_safe_uav = 0.7
d_communication = 20
optimal_last = np.zeros(3)

def object_fun(x):
	Pcen = (P1+P2+P3)/3
	return 1/((Pcen[0] - Pb[0] - x[0])**2)# + (Pcen[1] - Pb[1] - x[1])**2 + (Pcen[2] - Pb[2] - x[2])**2)
# + ((x[0] - bearing_cmd_vel.linear.x)**2 + (x[1] - bearing_cmd_vel.linear.y)**2 + (x[2] - bearing_cmd_vel.linear.z)**2)
...
def	qpsolver():
	global bearing_cmd_vel,optimal_last
	...
	bearing_cmd_vel.linear.x = optimal[0]
	bearing_cmd_vel.linear.y = optimal[1]
	bearing_cmd_vel.linear.z = optimal[2]

	px4_bearing.vel_control(bearing_cmd_vel)
	optimal_last = optimal[:3]
